...
<template>
  <div class="app-container">
    ...
    [#if field.asType=='String']
      <el-input v-model="listQuery.${field.columnNameFirstLower}" placeholder="${field.columnAlias}" clearable style="width: 140px;" class="filter-item"  />
    [#elseif field.asType=='Long']
      <el-input v-model.number="listQuery.${field.columnNameFirstLower}" type="number" placeholder="${field.columnAlias}" clearable style="width: 100px;" class="filter-item"  />
    [#elseif field.asType=='Integer']
      <el-input v-model.number="listQuery.${field.columnNameFirstLower}" type="number" placeholder="${field.columnAlias}" clearable style="width: 100px;" class="filter-item"  />
    [#elseif field.asType=='Boolean']
      <el-select v-model="listQuery.${field.columnNameFirstLower}" placeholder="是否${field.columnAlias}" clearable class="filter-item" style="width: 140px">
        <el-option v-for="item in boolOptions" :key="item.k" :label="item.v" :value="item.k" />
      </el-select>
    [#elseif field.asType=='Enum']
      <el-select v-model="listQuery.${field.columnNameFirstLower}" placeholder="${field.columnAlias}" clearable class="filter-item" style="width: 140px">
        <el-option v-for="item in ${field.columnNameFirstLower}Options" :key="item.k" :label="item.v" :value="item.k" />
      </el-select>
    ...
    [#else]
      <el-input v-model="listQuery.${field.columnNameFirstLower}" placeholder="${field.columnAlias}" style="width: 100px;" clearable class="filter-item"/>
    [/#if]
    [/#list]
    ...
    [#if field.asType=='String']
      <el-table-column label="${field.columnAlias}" width="120px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.${field.columnNameFirstLower} }}</span>
        </template>
      </el-table-column>
    [#elseif field.asType=='Long']
    ...
    [#elseif field.asType=='Boolean']
      <el-table-column label="${field.columnAlias}" width="100px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.${field.columnNameFirstLower} | booleanFilter }}</span>
        </template>
      </el-table-column>
    [#elseif field.asType=='Enum']
      <el-table-column label="${field.columnAlias}" width="60px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.${field.columnNameFirstLower} | ${field.columnNameFirstLower}Filter }}</span>
        </template>
      </el-table-column>
    [#elseif field.asType=='Date']
    ...
    [#if field.asType=='String']
              <el-col span=8 align="center">
                <el-input v-model="listQuery.${field.columnNameFirstLower}" placeholder="${field.columnAlias}" clearable />
              </el-col>
    [#elseif field.asType=='Long']
              <el-col span=8 align="center">
                <el-input v-model.number="listQuery.${field.columnNameFirstLower}" type="number" placeholder="${field.columnAlias}" clearable />
              </el-col>
    [#elseif field.asType=='Integer']
              <el-col span=8 align="center">
                <el-input v-model.number="listQuery.${field.columnNameFirstLower}" type="number" placeholder="${field.columnAlias}" clearable  />
    ...
    [#elseif field.asType=='Boolean']
              <el-col span=8 align="center">
                <el-select v-model="listQuery.${field.columnNameFirstLower}" placeholder="是否${field.columnAlias}" clearable>
                  <el-option v-for="item in boolOptions" :key="item.k" :label="item.v" :value="item.k" />
                </el-select>
              </el-col>
    [#elseif field.asType=='Enum']
              <el-col span=8 align="center">
                <el-select v-model="listQuery.${field.columnNameFirstLower}" placeholder="${field.columnAlias}" clearable >
                  <el-option v-for="item in ${field.columnNameFirstLower}Options" :key="item.k" :label="item.v" :value="item.k" />
                </el-select>
              </el-col>
    [#elseif field.asType=='Date']
    ...
    [#list table.columns as field]
    [#if field.asType=='String']
        <el-form-item label="${field.columnAlias}" prop="${field.columnNameFirstLower}">
          <el-input v-model="temp.${field.columnNameFirstLower}" />
        </el-form-item>
    [#elseif field.asType=='Long']
        <el-form-item label="${field.columnAlias}" prop="${field.columnNameFirstLower}">
          <el-input type="number" v-model.number="temp.${field.columnNameFirstLower}" />
        </el-form-item>
    [#elseif field.asType=='Integer']
        <el-form-item label="${field.columnAlias}" prop="${field.columnNameFirstLower}">
          <el-input type="number" v-model.number="temp.${field.columnNameFirstLower}" />
    ...
    [#elseif field.asType=='Boolean']
        <el-form-item label="${field.columnAlias}" prop="${field.columnNameFirstLower}">
          <el-checkbox v-model="temp.${field.columnNameFirstLower}">
          ${field.columnAlias}
          </el-checkbox>
        </el-form-item>
    [#elseif field.asType=='Enum']
        <el-form-item label="${field.columnAlias}" prop="${field.columnNameFirstLower}">
          <el-select v-model="temp.${field.columnNameFirstLower}" class="filter-item" placeholder="请选择">
            <el-option v-for="item in ${field.columnNameFirstLower}Options" :key="item.k" :label="item.v" :value="item.k" />
          </el-select>
        </el-form-item>
    [#elseif field.asType=='Date']
        <el-form-item label="${field.columnAlias}" prop="${field.columnNameFirstLower}">
          <el-date-picker v-model="temp.${field.columnNameFirstLower}" type="datetime" placeholder="选择一个日期" />
        </el-form-item>
    [#elseif field.asType=='BigDecimal']
        <el-form-item label="${field.columnAlias}" prop="${field.columnNameFirstLower}">
          <el-input type="number" v-model.number="temp.${field.columnNameFirstLower}" />
...
const ${column.columnNameFirstLower}Options = [
	[#list column.enumList as e]
  { k: ${e.enumKey}, v: '${e.enumDesc}' }[#if e_has_next],[/#if]

	[/#list]
]
const ${column.columnNameFirstLower}KV = ${column.columnNameFirstLower}Options.reduce((acc, cur) => {
  acc[cur.k] = cur.v
  return acc
}, {})
...
const fieldOptions = [
  [#list table.columns as column]
  { k: '${column.columnNameFirstLower}', v: '${column.columnAlias}', t: '${column.asType}' },
  [/#list]
  { k: 'id', v: 'ID', t: 'Integer' }
]
...
[#list table.columns as column]
	[#if column.asType=='Enum']
    ${column.columnNameFirstLower}Filter(k) {
      return ${column.columnNameFirstLower}KV[k]
    },
	[#elseif column.asType=='Object']
	[#else]
	[/#if]
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        size: 20,
    [#list table.columns as field]
        ${field.columnNameFirstLower}: null,
    [/#list]
        andContent: null,
        orContent: null,
        sorts: null,
...
[#list table.columns as column]
	[#if column.asType=='Enum']
      ${column.columnNameFirstLower}Options: ${column.columnNameFirstLower}Options,
	[#elseif column.asType=='Object']
	[#else]
	[/#if]
[/#list]
    ...
    [#list table.columns as field]
    [#if field.asType=='String']
        ${field.columnNameFirstLower}: [{ required: true, message: '${field.columnAlias}必填', trigger: 'blur' }],
    [#elseif field.asType=='Long']
        ${field.columnNameFirstLower}: [{ type: 'integer', required: true, message: '请填写数字', trigger: 'blur' }],
    [#elseif field.asType=='Integer']
        ${field.columnNameFirstLower}: [{ type: 'integer', required: true, message: '请填写数字', trigger: 'blur' }],
    [#elseif field.asType=='BigDecimal']
        ${field.columnNameFirstLower}: [{ type: 'float', required: true, message: '请填写数字', trigger: 'blur' }],
    [#elseif field.asType=='Boolean']
        ${field.columnNameFirstLower}: [{ type: 'boolean', required: true, message: '请选择', trigger: 'change' }],
    [#elseif field.asType=='Enum']
        ${field.columnNameFirstLower}: [{ type: 'integer', required: true, message: '请选择', trigger: 'change' }],
    [#elseif field.asType=='Date']
        ${field.columnNameFirstLower}: [{ type: 'date', required: true, message: '请选择${field.columnAlias}', trigger: 'change' }],
    [#elseif field.asType=='Object']
    [#else]
        ${field.columnNameFirstLower}: [{ required: true, message: '${field.columnAlias}必填', trigger: 'blur' }],
    [/#if]
  ...
  methods: {
    ...
    [#list table.columns as field]
        ...
        ${field.columnNameFirstLower}: false,
        [#else]
        ${field.columnNameFirstLower}: null,
        [/#if]
    [/#list]
