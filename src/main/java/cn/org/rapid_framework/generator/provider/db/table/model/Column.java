...
public class Column implements java.io.Serializable,Cloneable{
	...
	public Column(Table table, int sqlType, String sqlTypeName,
		...
		if(sqlName.startsWith(BOOLEAN_PREFIX)){
			String temp = sqlName.substring(BOOLEAN_PREFIX.length());
			columnName = StringHelper.makeAllWordFirstLetterUpperCase(StringHelper.toUnderscoreName(temp));
		}else{
	...
	public Column(String javaName,String type,String remark) {
	   	String sqlName =  StringHelper.toUnderscoreName(javaName);
		setJavaType(type);
	   	if(type.equalsIgnoreCase("Boolean")){
			setSqlName(BOOLEAN_PREFIX + sqlName);
		}else{
	   		setSqlName(sqlName);
		}
		columnName = javaName;
	...
	public void setSqlName(String v) {
		...
		if(StringUtils.isNotBlank(_sqlName) && !v.equalsIgnoreCase(_sqlName)) {
...
//		asType = ActionScriptDataTypesUtils.getPreferredAsType(getJavaType());
		asType = getSimpleJavaType();
		//说明是复杂类型
		if(StringUtils.isBlank(JavaPrimitiveTypeMapping.getPrimitiveTypeOrNull(asType)) && !StringUtils.equalsIgnoreCase(asType,"String")&& !StringUtils.equalsIgnoreCase(asType,"Date")){
			if(StringUtils.containsIgnoreCase(asType,"Decimal")){
				asType = "BigDecimal";
			}
			else {
				asType = "Object";
		...
		if(StringUtils.isNotBlank(remark)){
			...
			if(remark.contains(":")){
				...
				//真正的字段中文注释
				columnAlias = remarkInfo[0];
			    if(remark.contains("|")) {
					asType = "Enum";
					enumClassName = getColumnNameFirstUpper()+"Enum";
					setEnumString(remarkInfo[1].replace("|",";"));
				}
			}else{
				columnAlias = remark;
			}
